...
#' @param cutoff An integer scalar. The value at which correlations should be declared "antonyms". 
...
psyAnt <- function(x, cutoff = -.60, cor.method = "pearson"){
  ...
  if(length(which(cor.matrix < 1.0 & cor.matrix <= cutoff)) == 0) stop ("No antonyms found at chosen cut-off level")
  ...
  sprintf("Number of antonyms found: %i", nrow(pairs))
